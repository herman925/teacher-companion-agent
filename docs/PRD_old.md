...
6. **Workbench-first delivery** ([ADR-0006](./adr/0006-workbench-first-plan-tree.md)) — the 工作台 plan tree (月→周→日) is the deliverable and the per-node dialogue is the v1 core capability; the chat is scoped to the 教研员 role (intake, judgement, explaining a node's basis, diagnosing an activity). Left/right split retained for v1; mobile is full-screen.
7. **Tiered context** ([ADR-0007](./adr/0007-tiered-context-and-change-propagation.md)) — per-turn context built from the turn's subject rather than the full state snapshot, with an engine-computed staleness marker when an edit invalidates neighbouring plan nodes. Format (TSV skeleton vs compact JSON) is provisional pending the token/cost measurement.
8. **Teacher interaction axes** ([ADR-0009](./adr/0009-teacher-interaction-axes.md)) — six scored dimensions replacing the single response-style selector, with cross-course teacher memory.
9. **Conversation model** ([ADR-0010](./adr/0010-conversation-and-workbench-model.md)) — one message log per course with a per-message subject tag, so a node conversation is a filtered view rather than a separate thread. The 工作台 looks and navigates; all input happens on the left. Per-node 批注 and the ✓确认 click are absorbed; confirmation becomes a quoted engine event.
10. **Four memory scopes and the class object** ([ADR-0011](./adr/0011-memory-scopes-and-serialization.md)) — node, course, class and teacher memory, auto-extracted with an undoable receipt and never a form. A named class is a new object distinct from the age-band field, so constraints like 「班上没有鼓」 survive into the next theme with the same children.

### 5.2 Out of scope (deferred)

- **Regional 教研 data warehouse and leader-facing analytics** — the second audience (front-end for teachers, back-end for leadership) is the stated commercial driver but ships after v1. Its data model is *not* deferred: any aggregation touching child observations needs a compliance decision recorded as an ADR before collection starts (non-negotiable #4).
- **主题预设网络图 and `核心驱动问题` as required v1 content** — both remain reachable capabilities, neither is mandatory ([ADR-0008](./adr/0008-v1-scope-amendment.md)).
...
6. Context serialization and cost — tiered context is adopted ([ADR-0007](./adr/0007-tiered-context-and-change-propagation.md)); the wire format is not. TSV skeleton, compact JSON, and today's pretty-printed full state are measured against each other on real course data (prompt tokens per course turn and per node turn, cache-hit rate, API cost per completed course) before the format is frozen.
7. ~~Staleness clearing~~ — ANSWERED ([ADR-0011](./adr/0011-memory-scopes-and-serialization.md) §8): the badge names what changed upstream and why; it clears on 跟着改, 我自己改 or 这样就行, never on merely opening the node; it retires on its own once the node's dates have passed.
7a. Is 月计划 a document teachers submit on a monthly cycle? [ADR-0010](./adr/0010-conversation-and-workbench-model.md) §9 makes the root a *phase* of 2–5 weeks rather than a calendar month, which matches 锋's 2—3 week Stage 1. If teachers hand a monthly plan to the 园长 on a fixed cycle, that decision reopens. Needs a teacher, not a design debate.
7b. ~~Who owns the activity content schema~~ — ANSWERED by the 2026-07-28 transcript: 林朝湃 owns it, and also owns distilling ≤10 深度学习 principles for the system prompt. 陈栩锋 owns a separate artifact, the corrected dialogue-flow write-up. The node detail view is blocked on 林朝湃's schema, not on a conflict.
7c. Does a node conversation carry its own earlier history, or start clean from the plan artifacts? 林朝湃 argued for clean (「历史对话就没有了」); [ADR-0010](./adr/0010-conversation-and-workbench-model.md) §1a keeps the history in storage but leaves what is *sent to the model* open. Settle with 林朝湃 and 陈栩锋 before build.
8. Axis inference reliability — which behavioral signals are trustworthy enough to move a teacher's interaction axes? Rewriting a delivered plan may mean 方案开放度 is too low, or simply that the plan was wrong. If pilots report drift, inference ships off by default and the manual handles ship alone.
9. Two-audience data model — how long must teacher usage accumulate before it supports regional 教研 decisions, and what is the phase-1 acceptance point? Front-end schema choices lock this, so it cannot wait for the back-end phase.
10. Pedagogy depth without gates — with the network map optional and the driving question unforced ([ADR-0008](./adr/0008-v1-scope-amendment.md)), nothing blocks a thin v1 course. Watch pilot plan quality; the intended remedy is upstream content (canon injection, the activity content schema), not restored gates.
11. 预设/生成 rebalance — direction ADOPTED via [ADR-0003](./adr/0003-blueprint-first-planning-lens.md) (blueprint-first planning lens, 锋's Stage-1 five steps, two-round delivery; Tier-0 shipped in the demo: client-numbered collapsible blueprint + PC 导图 view). Still open and empirical: the "sweet spot" — how complete a blueprint to generate from how little input — to be answered by pilot measurement (turns-to-first-blueprint, questions per turn, share of `ai_suggestion` vs `confirmed`), not design debate.
