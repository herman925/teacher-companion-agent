...
5. Timeout physics (reworked 2026-07-23 with Herman after a healthy ~8-min generation died to the old flat abort). Three timers on streaming calls, one on buffered calls:
   - **Idle 120s** (streaming) — the graceful guard. Every vendor byte re-arms it; a productive stream is never cut, only one that goes silent. Idle-dead primaries hop `altBaseURLs`.
   - **Thinking budget 5 min** (streaming) — the "answer now" analog (ChatGPT's button ends the reasoning phase server-side; `chat/completions` has no mid-request steer channel, so we abort-and-redirect). Armed until the first answer-content delta. On expiry: ONE forced-answer retry — a new request carrying the vendor thinking switch off where documented (`disableThinking`; GLM/Z.AI `thinking:{type:"disabled"}`) **plus the model's own captured reasoning draft** (tail 6000 chars of streamed `reasoning_content`/`<think>` text) framed as 「直接采用其结论，不要重新推理」, so the burned minutes carry over instead of restarting from zero. Retry has no budget; if it also fails, the timeout surfaces — never a third attempt. Aggregators without a documented switch get the draft+nudge only.
   - **Total ceiling 10 min wall-clock across both attempts** (5 thinking + ~5 forced answer; the retry's ceiling is the remaining time, floor 60s). Sole guard on non-streaming calls. Every proxy in front must read longer than the idle window — the public nginx reads at 660s ≫ 120s — so the abort is always ours and reports through the stream.
   - Error kind `timeout`, `.phase: 'idle'|'thinking'|'total'`, distinct teacher-readable messages. A `total` timeout stops the provider failover chain (the wait is never paid twice); `idle` still hops.
   - **Pending real-key test (next session)**: `thinking:{type:"disabled"}` is docs-verified (docs.bigmodel.cn thinking-mode) but not yet exercised against live GLM; confirm the forced retry answers fast and the draft handoff doesn't break `json_schema` output.
...
7b. Prompt caching (2026-07-23): every provider we call runs AUTOMATIC prefix caching (MiniMax cache-hit tier ¥0.42–?, Kimi context caching ¥1.1, GLM implicit) — nothing to call, but hits require a byte-stable token prefix, which the old assembly destroyed by embedding the per-turn `course_state` snapshot in `messages[0]`. Now (`buildPromptParts` + `cacheStableHistory`): messages = `[static system (base+contract+stage+档案)] … history … [state-note system] [user]`. The snapshot changes freely at the tail; the rules and all history before the last exchange stay cache-hot, and the L4 retry re-sends a 100%-cached prefix. History trims in 12-message blocks (window 24–35) instead of sliding by 2 every turn, so 5 of 6 turns extend the prefix instead of moving it. **Watch `usage` in the debug drawer for vendors' cached-token counters to confirm live hits; also confirm each vendor accepts a second mid-conversation `system` message** (OpenAI-compatible norm, but verify with real keys).
...
9. **联网搜索 — GLM/Z.AI only, and run by us, not by the model** (2026-07-29, [ADR-0012](adr/0012-runtime-harness-after-the-workflow.md) §6; implemented in [demo/src/web-search.mjs](../demo/src/web-search.mjs)). Three findings forced this shape:
   - **MiniMax has no web search in its chat API.** The OpenAI-compatible spec supports exactly one tool type, `function`; its search product is a separate MCP server. There is nothing to switch on. Our MiniMax path also forces `tool_choice` to `emit_turn`, which structurally forbids any other tool call — so even a function-shaped search tool could not fire without a second request.
   - **GLM's in-chat `{"type":"web_search"}` tool is undocumented against `response_format: json_schema`.** Not guessed at; avoided.
   - **Both Zhipu regions publish a STANDALONE endpoint**, which sidesteps all of the above and works identically whatever the jsonStrategy: `POST ${baseURL}/web_search`, Bearer auth, body `{search_query, search_engine, search_intent, count, content_size}`, response `{search_result:[{title, content, link, media, publish_date, refer}]}`. Engine ids differ by region — mainland `search_std` (¥0.01/call) · `search_pro` (¥0.03) · `search_pro_sogou`/`search_pro_quark` (¥0.05); international `search-prime` (no published per-call price). `search_query` max 70 chars, `count` 1–50. We default to the cheapest engine and 5 results.
   - **We compose the query and we choose the moment.** Retrieval fires only on intake (`stage ≤ 1`) with the teacher's toggle on, so search reads the 主题 for her instead of becoming a general query box — the 长沙天气 case ADR-0012 exists to prevent. Results are injected as a system message *after* the cached prefix, so an occasional search never busts prefix caching. A search failure is never fatal: it degrades to no context and the turn proceeds, with a `web_search` report on the turn either way.
   - Every other provider reports the capability as unavailable **with a reason** rather than showing a dead switch. Kimi's `$web_search` builtin is real but NOT wired — the UI previously implied it was.
   - **Pending real-key test**: the endpoint shape is docs-verified for both regions and the wiring is verified end-to-end against a live 401 from bigmodel.cn, but no successful search has run yet. Confirm result quality and that `content_size: medium` summaries are worth their tokens.
