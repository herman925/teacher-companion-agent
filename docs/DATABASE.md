...
| `GET /admin` · `GET/DELETE /api/admin/*` | Token-gated data console (`admin.html`): list all courses with message/snapshot counts, view a full record, export all, delete/multi-delete. `ADMIN_TOKEN` env gates the API (unset = open, acceptable only on tunnel-protected dev). See OPERATIONS.md "Inspecting demo data". |
