...
5. Context compression (Headroom, github.com/headroomlabs-ai/headroom) — revisit once model calls run server-side and token spend is measured. Deferred 2026-07-21: Chinese-prose compression quality unverified, lossy compression sits inside the evidence trust path, and `blueprint_delta` already covers the largest repeat cost. If trialed: proxy mode on the VM, JSON/tool-output compression only (no prose model), compare turn-contract pass rate and evidence-citation accuracy on Chinese fixtures compressed vs raw.
6. Web search (Z.ai / Zhipu, docs.z.ai/guides/tools/web-search) — phase-2 idea at best. Deferred 2026-07-21: the in-chat `web_search` tool is Zhipu-proprietary (breaks adapter portability) and injects unvetted live web content toward teacher-facing output (culture stays backstage; generative-AI compliance surface grows). If ever adopted: the standalone Web Search API only, called server-side by our proxy as a harness-controlled tool (e.g. 番禺 local-culture background research), results filtered before reaching the teacher. Pricing unverified.
