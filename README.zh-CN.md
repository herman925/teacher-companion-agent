...
English version: [README.md](README.md) · 原工作名「教师资源发展平台」（2026-07-16 更名）
...
演示：运行 `node demo/serve.mjs`，打开输出的地址。演示模式完全离线可用（五条入口流程——从零陪跑、已有主题优化、过程中续聊、课程故事整理、素材支持——各自走不同的工作流节点）。要接真实模型，先部署代理（见 [docs/DEPLOY.md](docs/DEPLOY.md)），在设置中填写服务器地址，再粘贴 MiniMax/GLM/Kimi 的 API 密钥。勾选「开发者模式」后，每一轮回应都会标注所应用的 V1.3 节点与原则，调试抽屉里还有阶段 0-5 的工作流地图。
