...
- **Runtime harness** (`demo/src/harness.mjs`) governs the LLM at runtime. Since [ADR-0012](docs/adr/0012-runtime-harness-after-the-workflow.md) it guards three things: the **scope shell** (this tool builds preschool theme-inquiry courses; off-purpose use is refused before the call, which is also how we stop burning tokens on it), **evidence and provenance** (non-negotiable #1, plus citation-backed confirmation), and **structural integrity** (the turn parses, deltas are well-formed). [ADR-0014](docs/adr/0014-one-question-per-revision-turn.md) added a fourth, **interaction economy**: once the plan tree exists, a turn may ask at most one question, and after she answers it the tree must move. Stage gates and node prerequisites lapsed with the workflow chain. It constrains *the model*, never the teacher — and inside the shell the model plays freely.
