...
- 每个节点可以带 `rationale`（这个节点为什么存在，教师点开节点就能看到）：`{"heard": [{"quote": "教师原话片段"}], "assumed": "你的推断", "pedagogy": "推断背后的教学依据", "profile_basis": "来自教师档案的哪条信息"}`。教师说过的写进 heard（引用原话，不改写）；你猜的写进 assumed + pedagogy。有据可写就写——蓝图同时是专业成长材料。
