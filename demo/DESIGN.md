...
* **用户中心:** a people icon in the header opens the same centered-modal pattern as settings, left nav 账号 / 教师档案 / 登录设备. 账号: display name (rules stated gently inline: 全站唯一、半年可改一次、有敏感词过滤) and password change. **教师档案 lives here when signed in** (server-stored, follows the account); the settings modal then hides its own 档案 tab and points here — one editing home at a time, never two. Signed out, the profile stays in settings as the local-only path. 登录设备: session list with per-device 退出 (brick). Same warm family throughout.
* **回应风格 is a promise, not a label:** each of the five styles maps to an explicit directive injected into the system prompt (single-sourced in `prompt-builder.mjs`), and the UI shows the exact directive as the explanation under the select (and as option tooltips) — the teacher reads precisely what the model is told to do.
