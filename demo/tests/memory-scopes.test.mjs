...
import {
  SCOPES, MEMORY_COLUMNS, MEMORY_TSV_VERSION, DEFAULT_FACT_CAP,
  normalizeFacts, mergeFact, supersedeFact, capFacts, factsToTSV, widenScope,
...
const NO_DRUM = {
  id: 'f-drum', scope: 'class', text: '班上没有鼓',
  quote: '我们班没有鼓，而且有几个孩子很怕大声', at: AT(1), source: 'teacher', ...WIDENED,
};
const LOUD = {
  id: 'f-loud', scope: 'class', text: '有几个孩子怕大声',
  quote: '我们班没有鼓，而且有几个孩子很怕大声', at: AT(1), source: 'teacher', ...WIDENED,
};
/** The same constraint before she widened it: what auto-extraction can write. */
const DRUM_COURSE = {
  id: 'f-drum-c', scope: 'course', text: '班上没有鼓',
  quote: '我们班没有鼓，而且有几个孩子很怕大声', at: AT(1), source: 'auto',
};
const THEME = {
  id: 'f-theme', scope: 'course', text: '这次主题定在东乡龙舟',
  quote: '我们想做东乡龙舟', at: AT(2), source: 'auto',
};
...
test('normalize: the safe shape is always complete', () => {
  const [f] = normalizeFacts({ text: '班上没有鼓' }, NOW);
  for (const k of ['scope', 'text', 'quote', 'at', 'id']) assert.ok(k in f, `缺字段：${k}`);
...
test('normalize: an unknown scope defaults narrow, never broad', () => {
  ...
  assert.equal(normalizeFacts({ text: 'x', scope: 'kindergarten' }, NOW)[0].scope, 'course');
  assert.equal(normalizeFacts({ text: 'x', scope: '' }, NOW)[0].scope, 'course');
  assert.equal(normalizeFacts({ text: 'x' }, NOW)[0].scope, 'course');
});
...
test('normalize: node scope is accepted, never invented', () => {
  ...
  assert.equal(normalizeFacts({ text: 'x', scope: 'node', source: 'teacher' }, NOW)[0].scope, 'node');
  assert.ok(SCOPES.includes('node'));
});

test('normalize: a fact with no text is dropped, not filed empty', () => {
  const out = normalizeFacts([{ text: '   ' }, { text: '' }, {}, null, 'nonsense', { text: '真的事实' }], NOW);
...
test('normalize: ids are stable across reloads, and never collide', () => {
  const a = normalizeFacts({ text: '班上没有鼓' }, NOW)[0].id;
  const b = normalizeFacts({ text: '班上没有鼓' }, NOW)[0].id;
  assert.equal(a, b, 'id 要能让 superseded_by 指针在重新加载后依然有效');
  const dup = normalizeFacts([{ id: 'x', text: 'a' }, { id: 'x', text: 'b' }], NOW);
  assert.notEqual(dup[0].id, dup[1].id);
});

test('normalize: curation bookkeeping survives a round trip through storage', () => {
  const stored = {
    id: 'f1', scope: 'class', text: 'x', quote: 'q', at: AT(1), source: 'teacher',
...
test('normalize: an ordinary fact stays lean', () => {
  ...
  const [f] = normalizeFacts({ text: 'x', at: AT(1) }, NOW);
  assert.deepEqual(Object.keys(f).sort(), ['at', 'id', 'quote', 'scope', 'source', 'text']);
});
...
test('INVARIANT: auto-extraction claiming class scope lands at course', () => {
  const [f] = normalizeFacts({ text: '班上没有鼓', scope: 'class', source: 'auto' }, NOW);
  assert.equal(f.scope, 'course', '自动抽取只能写 course');
});

test('INVARIANT: auto-extraction claiming teacher scope lands at course', () => {
  const [f] = normalizeFacts({ text: '她喜欢先看整月', scope: 'teacher', source: 'auto' }, NOW);
...
test('INVARIANT: an unlabelled fact is treated as the machine guessing', () => {
  ...
  const [f] = normalizeFacts({ text: '班上没有鼓', scope: 'teacher' }, NOW);
...
test('INVARIANT: the clamp bites on reload too, not only on first write', () => {
  ...
  const stored = [{ id: 'f1', scope: 'teacher', text: '班上没有鼓', at: AT(1), source: 'auto' }];
  assert.equal(normalizeFacts(stored, NOW)[0].scope, 'course');
});

test('INVARIANT: no curation function can widen — only widenScope can', () => {
  const claim = { id: 'f-new', scope: 'teacher', text: '班上没有鼓', at: AT(3), source: 'auto' };
...
test('INVARIANT: an incoming fact cannot mint teacher provenance for itself', () => {
  const [f] = normalizeFacts({ text: '班上没有鼓', scope: 'class', source: 'teacher' }, NOW);
...
test('INVARIANT: a fabricated child reaction cannot be filed as memory', () => {
  // Memory rides EVERY turn, whole, forever. A child claim filed here is
  // non-negotiable #1 with no expiry date, so it is archived on arrival —
  // kept and visible in the export, never rendered into the prompt.
  const [f] = normalizeFacts({ text: '孩子们已经学会了划桨的动作', quote: '（模型编的）' }, NOW);
  assert.equal(f.archived, true);
...
test('MUST PASS — an ordinary class constraint is untouched by that screen', () => {
  ...
  const [loud] = normalizeFacts({ text: '有几个孩子很怕大声' }, NOW);
...
test('merge: a restatement updates the timestamp instead of appending', () => {
  const again = { scope: 'course', text: '班上没有鼓。', quote: '跟你说过我们班没有鼓', at: AT(9) };
...
test('merge: the merged row keeps the original id and quote', () => {
  const again = { scope: 'course', text: '班上没有鼓', quote: '又说了一遍', at: AT(9) };
...
test('merge: punctuation and spacing do not make a new fact', () => {
  for (const text of ['班上没有鼓', '班上没有鼓，', '班上 没有鼓。', '班上没有鼓！']) {
    const r = mergeFact([DRUM_COURSE], { scope: 'course', text, at: AT(9) }, NOW);
...
test('merge: an extracted restatement never reaches the class fact she widened', () => {
  ...
  const r = mergeFact([NO_DRUM], { scope: 'class', text: '班上没有鼓', at: AT(9), source: 'teacher' }, NOW);
...
test('merge: the same words at a different scope are a different claim', () => {
  ...
  const courseCopy = { scope: 'course', text: '班上没有鼓', at: AT(9) };
...
test('merge: a paraphrase is added — the deliberate direction to fail in', () => {
  ...
  const r = mergeFact([NO_DRUM], { scope: 'class', text: '我们班没有鼓', at: AT(9), source: 'teacher' }, NOW);
  assert.equal(r.action, 'added');
});
...
test('merge: an archived fact is not a merge target', () => {
  ...
  const r = mergeFact([retired], { scope: 'class', text: '班上没有鼓', at: AT(9), source: 'teacher' }, NOW);
  assert.equal(r.action, 'added');
...
test('supersede: the old fact is archived with a pointer, never deleted', () => {
  const bought = {
    id: 'f-bought', scope: 'class', text: '园里买了两个鼓', quote: '园里上周买了两个鼓',
...
test('supersede: an unrelated fact at the same scope is untouched', () => {
  const bought = { id: 'f-bought', scope: 'class', text: '园里买了两个鼓', at: AT(20), source: 'teacher', supersedes: 'f-drum' };
  const r = supersedeFact([NO_DRUM, LOUD], bought, { ...NOW, byTeacher: true });
...
test('supersede: with no pointer, nothing is archived and both stay live', () => {
  ...
  const r = supersedeFact([NO_DRUM], { scope: 'class', text: '园里买了两个鼓', at: AT(20), source: 'teacher' }, NOW);
...
test('supersede: an automatic judgement cannot retire a deliberate one', () => {
  ...
  const guess = { id: 'f-guess', scope: 'class', text: '园里买了两个鼓', at: AT(20), source: 'auto', supersedes: 'f-drum' };
...
test('supersede: she can retire her own class fact by hand', () => {
  const hers = { id: 'f-hers', scope: 'class', text: '园里买了两个鼓', at: AT(20), source: 'teacher', supersedes: 'f-drum' };
...
test('supersede: the same words WITHOUT her tap cannot retire her class fact', () => {
  ...
  const claimed = { id: 'f-x', scope: 'class', text: '园里买了两个鼓', at: AT(20), source: 'teacher', supersedes: 'f-drum' };
...
test('supersede: auto may retire an equally narrow fact', () => {
  const newTheme = { id: 'f-t2', scope: 'course', text: '主题改成中秋灯笼', at: AT(20), source: 'auto', supersedes: 'f-theme' };
  const r = supersedeFact([THEME], newTheme, NOW);
...
test('supersede: an already-archived or missing target is reported, not silent', () => {
  ...
  const next = { id: 'f-x', scope: 'class', text: '有鼓了', at: AT(20), source: 'teacher', supersedes: ['f-drum', 'f-ghost'] };
...
const many = (n, from = 1) => Array.from({ length: n }, (_, i) => ({
  id: `f${i + from}`, scope: 'course', text: `事实${i + from}`, at: AT(i + from), source: 'auto',
}));
...
test('cap: oldest-UNUSED goes first, not oldest', () => {
  ...
  // touched again.
  const facts = [
    { id: 'old-but-used', scope: 'course', text: '常用的老事实', at: AT(1), used_at: AT(28), source: 'auto' },
    { id: 'newer-unused', scope: 'course', text: '没再用过', at: AT(5), source: 'auto' },
    { id: 'newest', scope: 'course', text: '最新的', at: AT(9), source: 'auto' },
  ];
...
test('cap: archived facts do not count toward the cap', () => {
  ...
  const facts = [
    ...many(3),
    { id: 'gone', scope: 'course', text: '早就归档了', at: AT(1), source: 'auto', archived: true, archived_at: AT(2), archive_reason: 'cap' },
  ];
...
test('cap: never widens a fact on the way through', () => {
  const r = capFacts([...many(3), { id: 'w', scope: 'class', text: '想变宽', at: AT(9), source: 'auto' }], 10, NOW);
...
test('widen: node scope is not this module to promote', () => {
  const nodeFact = { id: 'n1', scope: 'node', text: '因为孩子提了这个问题', at: AT(1), source: 'teacher' };
...
test('widen: nothing above teacher scope, and a no-op says so', () => {
  const top = { id: 't1', scope: 'teacher', text: '她喜欢先看整月', at: AT(1), source: 'teacher', ...WIDENED };
...
test('tsv: never emits an empty cell', () => {
  const bare = [{ id: 'f1', scope: 'course', text: '没有原话的事实', at: AT(1), source: 'auto' }];
...
test('tsv: filters to one scope, and renders all scopes without one', () => {
  ...
  assert.ok(!classOnly.includes('东乡龙舟'), 'course 的事实不该混进 class 块');
...
test('tsv: an over-length fact is reported, never truncated mid-qualifier', () => {
  const wordy = [{
    ...
    text: `${'很长的事实'.repeat(20)}，这一条是我的猜测，还没问过她`,
...
test('MUST PASS — a short fact renders exactly as before', () => {
  ...
  assert.deepEqual(row, ['f-theme', 'course', '这次主题定在东乡龙舟', '我们想做东乡龙舟', '2026-09-02', 'auto']);
});

test('tsv: a tab or newline inside a fact cannot break alignment', () => {
  const nasty = [{ id: 'f1', scope: 'course', text: '班上\t没有鼓\n还怕大声', quote: 'a\tb', at: AT(1), source: 'auto' }];
...
test('the drum story: one sentence in 活动 3.2.1 reaches 周3 two days later', () => {
  ...
  const heard = { text: '班上没有鼓', quote: '我们班没有鼓，而且有几个孩子很怕大声', at: AT(1), source: 'auto', scope: 'class' };
